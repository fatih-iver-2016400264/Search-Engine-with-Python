        ...
        for page in graph:
            ...
            for node in graph:
                if page in graph[node]:
                    if not is_reciprocal_link(graaph, node , page , k):
...
def is_there_reciprocal_link(url, urls, ranks, k):
    
    if k < 0:
        return False
    
    if k >= 0 and url in urls:
        return True
    
    is_there_collusion = False    
    
    for temp_url in urls:
        if temp_url not in ranks:
            continue
        else:
            is_there_collusion = is_there_reciprocal_link(url, ranks[temp_url], ranks, k-1)
            if is_there_collusion:
                return True
                
    return is_there_collusion
