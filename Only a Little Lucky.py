...
def ordered_search(index, ranks, keyword):
    ...
    if keyword in index:
        ...
        for url in search_result:
            ...
            rank = ranks[url]
            search_result_ranks.append(rank)
            
            if rank in ranks_map_urls:
                ranks_map_urls[rank].append(url)
            else:
                ranks_map_urls[rank] = [url]
                
        ordered_search_result_ranks = quicksort(search_result_ranks)
        
        ordered_search_result = []
        
        for rank in ordered_search_result_ranks:
            
            mapped_urls = ranks_map_urls[rank]
            
            ordered_search_result.append(mapped_urls.pop())
                
        return ordered_search_result
